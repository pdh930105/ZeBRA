...
import torch
sys.path.append(os.path.abspath("./BFA/"))
...
def weight_bit_check(net, n_bits):
    layer_weight_bit = {}
    layer_info = {}
    total_weight_bit = torch.zeros([n_bits])
    for name, m in net.named_modules():
        if isinstance(m, quan_Conv2d) or isinstance(m, quan_Linear):
            m.__reset_stepsize__()
            m.__reset_weight__()
            
            bin_weight = int2bin(m.weight.detach().view(-1), m.N_bits).short()
            binary_bin_weight = (bin_weight.repeat(m.N_bits, 1).short() & m.b_w.abs().short()) // m.b_w.abs().short()
            count_bit = binary_bin_weight.sum(axis=1)
            layer_weight_bit[name] = count_bit
            layer_info[name] = len(bin_weight)
            total_weight_bit += count_bit
    
    return layer_weight_bit, layer_info, total_weight_bit